...
async function generateRecipesWithGemini() {
  ...
  if (!apiKey) {
    if (recipesTitle) recipesTitle.textContent = "Recettes (0)";
    if (recipesList) recipesList.innerHTML = "<p style='font-size: 0.85rem; color: #d9534f;'>Clé API manquante. Veuillez réessayer.</p>";
    return;
  }
...
Réponds UNIQUEMENT sous la forme d'un tableau JSON d'objets sans aucun texte autour, avec la structure suivante :
[
  {
  ...
  // Utilisation de la version stable v1 et du modèle officiel 2.5
  ...
  try {
    ...
    // Nettoyage du texte si le modèle entoure le JSON de balises ```json ... ```
    ...
    console.error(err);
    if (recipesList) recipesList.innerHTML = "<p style='font-size: 0.85rem; color: #d9534f;'>Erreur de génération ou format JSON invalide.</p>";
  }
}
