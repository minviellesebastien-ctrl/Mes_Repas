...
async function generateRecipesWithGemini() {
  ...
  try {
    ...
    if (data.error) {
      if (recipesList) recipesList.innerHTML = `<p style='font-size: 0.85rem; color: #d9534f;'>Erreur : ${data.error}</p>`;
      return;
    }

    generatedRecipes = data;
    renderRecipes(generatedRecipes);

  } catch (err) {
    console.error(err);
    if (recipesList) recipesList.innerHTML = "<p style='font-size: 0.85rem; color: #d9534f;'>Erreur de communication avec le serveur.</p>";
  }
}
