...
async function generateRecipesWithIA() {
  ...
  recipesList.innerHTML = "<p style='font-size: 0.85rem; color: #62826c;'>L'IA génère vos recettes uniques...</p>";

  const prompt = `Propose 5 idées de recettes faciles et très variées en français spécialement adaptées au mode de cuisson "${selectedMode}" en utilisant principalement ces ingrédients : ${userIngredients.join(", ")}. 
Format JSON STRICT attendu (sans markdown, sans explications) :
[
  {
    "title": "Nom précis de la recette",
    "prepTime": "15 min",
  ...
  try {
    ...
    if (jsonMatch) {
      ...
      recipesList.innerHTML = "<p style='font-size: 0.85rem; color: #62826c;'>Erreur de format d'IA. Réessayez.</p>";
    }
  } catch (err) {
    recipesList.innerHTML = "<p style='font-size: 0.85rem; color: #62826c;'>Erreur de connexion lors de la génération. Vérifiez votre réseau.</p>";
  }
}
