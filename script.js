// Récupération de la clé stockée dans le navigateur (Option A)
...
// Génération de recettes via l'alias universel gemini-flash
...
Adapte précisément les explications et étapes au mode "${selectedMode}".`;
  ...
  try {
    ...
    if (data.error) {
      // Si la clé est rejetée ou périmée, réinitialiser le stockage local
    ...
    console.error(err);
    if (recipesList) recipesList.innerHTML = "<p style='font-size: 0.85rem; color: #62826c;'>Erreur de connexion à l'IA.</p>";
  }
}
