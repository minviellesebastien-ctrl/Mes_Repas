...
// Base de connaissances pour le générateur dynamique
const COOKING_TECHNIQUES = {
  cookeo: {
    label: "Cookeo",
    time: "12 min",
    verbs: ["Faire dorér", "Cuire sous pression", "Mélanger avec le mode dorage"],
    steps: [
  ...
  airfryer: {
    label: "Air Fryer",
    time: "18 min",
    verbs: ["Rôtir à l'air chaud", "Dorér", "Casseroler à 180°C"],
    steps: [
      ...
      "Casseroler/cuire pendant 15 minutes en secouant le panier à mi-cuisson."
    ]
  },
  simple: {
    ...
    steps: [
      ...
      "Bisser le feu, ajouter les condiments et laisser mijoter à couvert pendant 12 minutes.",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Bascule Écrans & Génération locale
...
// Algorithme de génération intelligente de recettes
function generateDynamicRecipes() {
  ...
  // Création de 4 variations distinctes à partir des ingrédients de l'utilisateur
  for (let i = 0; i < 4; i++) {
    ...
    if (userIngredients.length === 1) {
      titleName = `${style.prefix} ${mainIng} ${style.suffix}`;
    } else {
      titleName = `${style.prefix} ${mainIng} et ${secIng} ${style.suffix}`;
    }
