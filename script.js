// Remplace par ta vraie clé API Gemini
...
// Appel à Gemini 1.5 Flash avec réponse structurée
...
Adapte précisément les explications et étapes au mode "${selectedMode}".`;
  ...
  try {
    ...
    if (data.error) {
      recipesList.innerHTML = `<p style='font-size: 0.85rem; color: #d9534f;'>Erreur Gemini : ${data.error.message}</p>`;
      return;
    }
